...
const Cards = ({ data }) => {
  console.log(data);
  return (
    <div className=" se:bg-black se:h-[400px]   se:w-[250px] xs:w-[400px] xs:bg-black xs:h-[300px]  se:items-center se:p-0 se:pr-0 pr-4 first xs:flex xs:flex-row xs:my-2 text-white xs:justify-center xs:items-center md:w-[600px] md:h-[400px] sm:w-[500px] sm:h-[350px] se:flex ">
      ...
      <div className=" content xs:w-full se:w-[200zpx] xs:h-full text-center items-center flex justify-center flex-col">
        ...
        <span className="xs:block text-left">
          <span className="  xs:block font-bold se:text-sm  xs:text-xl">
            {"We are HnCCians "}
          </span>
